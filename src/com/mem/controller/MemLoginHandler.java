		...
		if ("login".equals(action)) {
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req.getRequestDispatcher("/frontend/members/memLogin.jsp");
				...
				if (hasUsrid && (password.equals(map.get(usrid)))) {
					...
					try { /************ 3.若帳號密碼正確, 將會員資料存入session並轉回原網頁 ***************/
...
//										System.out.println("長度:" + memVO.getM_photo().length);
		...
		if ("forgetPsw".equals(action)) {
			...
			if (input == null || input.trim().length() == 0) {
				...
				} catch (Exception e) { // 查詢不到則回傳false
					out.print(false);
					e.printStackTrace();
				}
